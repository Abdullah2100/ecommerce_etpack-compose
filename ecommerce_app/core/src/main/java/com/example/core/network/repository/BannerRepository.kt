...
class BannerRepository(private val client: HttpClient, private val authDao: AuthDao) {
    ...
    suspend fun getRandomBanner(): NetworkCallHandler = withContext(Dispatchers.IO) {
        val authData =
            getAuthData() ?: return@withContext NetworkCallHandler.Error("user is not authenticate ")
    ...
    suspend fun deleteBanner(bannerId: UUID): NetworkCallHandler = withContext(Dispatchers.IO) {
        val authData =
            getAuthData() ?: return@withContext NetworkCallHandler.Error("user is not authenticate ")
